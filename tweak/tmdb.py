...
# Find the tmdbv3api package location:
# >>> import tmdbv3api
# >>> tmdbv3api.__file__
# '/volume1/servers/torcpdb/.venv/lib/python3.12/site-packages/tmdbv3api/__init__.py'
# >>> exit()
...
class TMDb(object):
    ...
    def __init__(self, obj_cached=True, session=None):
        if self.__class__._session is None or session is not None:
            ...
            self.__class__._session.mount("http://", adapter)
            self.__class__._session.mount("https://", adapter)
